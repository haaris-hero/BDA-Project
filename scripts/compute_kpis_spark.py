...
def main():
    ...
    try:
        ...
        print(f"✅ Loaded: Kitchen={kitchen_df.count()}, Rider={rider_df.count()}, Orders={orders_df.count()}")
        ...
        rider_efficiency = rider_df \
            .join(dim_rider, on="rider_id", how="left") \
            .join(dim_zone, on="zone_id", how="left") \
            .groupBy("rider_id", "vehicle_type", "rider_df.zone_id", "zone_name", "avg_traffic_level") \
            .agg(
        ...
        restaurant_perf = kitchen_df \
            .join(orders_df, on="order_id", how="inner") \
            .join(dim_restaurant, kitchen_df.restaurant_id == dim_restaurant.restaurant_id, how="left") \
            .groupBy("restaurant_id", "restaurant_name", "cuisine_type", "zone_id") \
            .agg(
        ...
        revenue_kpis.write.format("mongodb") \
            .mode("append") \
            .option("database", "food_delivery") \
            .option("collection", "kpi_revenue_metrics") \
            .save()
        
        cancellation_risk.write.format("mongodb") \
            .mode("overwrite") \
            .option("database", "food_delivery") \
            .option("collection", "kpi_cancellation_risk") \
            .save()
